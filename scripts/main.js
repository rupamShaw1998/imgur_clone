...
async function searchmovie() {
  try {
    ...
    let res = await fetch(
      `https://api.unsplash.com/search/photos?query=${query}&client_id=WcBzGRD5vIyINECmpONQF9RuUlXokPLoWxYOD2Vgjh0`
    );
...
function appendData(data) {
  ...
  data.map(e => {
      ...
      div.addEventListener("click",()=>{
        localStorage.setItem("Single_div", JSON.stringify(e));
        window.location.href="details.html"
      })
